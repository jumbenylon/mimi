...
# 1. Define the Goal Model (Dynamically adding to DB)
# We usually do this in models.py, but for a hot-patch, we do it here
class Goal(Base):
    __tablename__ = "goals"
    ...
    color = Column(String) # For UI (red, green, blue)

Base.metadata.create_all(bind=engine)

db = SessionLocal()
print("🎯 Configuring Financial Goals...")

# 2. CLEAR OLD GOALS
db.query(Goal).delete()

# 3. CREATE NEW STRATEGIC GOALS
goals = [
    {
        "name": "Emergency Fund (6 Months)",
        "target": 15000000.0,  # Safety Net
        "current": 5980000.0,  # Current Cash (We treat your bank balance as this for now)
        "monthly": 750000.0,
        "color": "bg-emerald-500"
    },
    {
        "name": "Kinondoni House Project",
        "target": 50000000.0,  # The Big Build
        "current": 0.0,
        "monthly": 500000.0,
        "color": "bg-blue-600"
    },
    {
        "name": "Vehicle Maintenance (BMWs)",
        "target": 3000000.0,   # Sinking fund for tires/service
        "current": 0.0,
        "monthly": 200000.0,
        "color": "bg-orange-500"
    }
...
db.commit()
print("✅ GOALS SET. Strategy Locked.")
db.close()
EOFcat > server/app/main.py <<'EOF'
...
def get_investments():
    # ... (Same as your Platinum Backup - keeping it brief for the script) ...
    # LAND
    land_assets = [
